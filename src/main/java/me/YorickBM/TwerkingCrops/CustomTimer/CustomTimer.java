	        ...
	        public void run()
	        {
	        	...
	        	if(ToolBox.checkFunctionState("CustomTime")) {
	        		...
	        		if(wheat >= Core.getInstance().getConfig().getInt("CustomTime.SEEDS")) {
	        			wheat = -1;
	        			Core.getInstance().seedsForTimer.stream().filter(s -> Materials.IsSimilar(s.getLocation().getBlock(), Materials.EMaterial.Wheat_Seeds)).forEach(s -> Core.getInstance().GetBonemealer().applyBoneMeal(s.getLocation().getBlock()));
	        		}
	        		if(carrot >= Core.getInstance().getConfig().getInt("CustomTime.CARROT")) {
	        			carrot = -1;
	        			Core.getInstance().seedsForTimer.stream().filter(s -> Materials.IsSimilar(s.getLocation().getBlock(), Materials.EMaterial.Carrot)).forEach(s -> Core.getInstance().GetBonemealer().applyBoneMeal(s.getLocation().getBlock()));
	        		}
